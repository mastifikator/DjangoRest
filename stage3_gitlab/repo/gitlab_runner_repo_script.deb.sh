...
main ()
{
  ...
  curl -sSf "${apt_config_url}" --proxy "http://bproxy.msk.mts.ru:3128"> $apt_source_path
  curl_exit_code=$?
  ...
  # import the gpg key
  curl -L "${gpg_key_url}" --proxy="http://bproxy.msk.mts.ru:3128" 2> /dev/null | apt-key add - &>/dev/null
  echo "done."
